...
var shouldMascotChangeVoteYes = 0
var shouldMascotChangeVoteNo = 0

for vote in shouldMascotChangeVotes {
    if vote == true {
        shouldMascotChangeVoteYes += 1
    } else {
        shouldMascotChangeVoteNo += 1
    }
}
...
if shouldMascotChangeVoteYes > shouldMascotChangeVoteNo {
    print("Mascot must be changed, because quantity of \"Yes-voters\" — \(shouldMascotChangeVoteYes), more then \"No-voters\" — \(shouldMascotChangeVoteNo)")
} else {
    print("Mascot stay, because quantity of \"Yes-voters\" — \(shouldMascotChangeVoteYes), less or equal \"No-voters\" — \(shouldMascotChangeVoteNo)")
}
/*:
...
var shouldInstallCoffeeVendingMachineVotesYes = 0
var shouldInstallCoffeeVendingMachineVotesNo = 0

for vote in shouldInstallCoffeeVendingMachineVotes {
    if vote == true {
        shouldInstallCoffeeVendingMachineVotesYes += 1
    } else {
        shouldInstallCoffeeVendingMachineVotesNo += 1
    }
}

if shouldInstallCoffeeVendingMachineVotesYes > shouldInstallCoffeeVendingMachineVotesNo {
    print("Coffee vending machine must be installed, because quantity of \"Yes-voters\" — \(shouldInstallCoffeeVendingMachineVotesYes), more then \"No-voters\" — \(shouldInstallCoffeeVendingMachineVotesNo)")
} else {
    print("Coffee vending machine can't be installed, because quantity of \"Yes-voters\" — \(shouldInstallCoffeeVendingMachineVotesYes), less or equal then \"No-voters\" — \(shouldInstallCoffeeVendingMachineVotesNo)")
}

shouldHaveMorePollOptionsVotes.count

var shouldHaveMorePollOptionsVotesYes = 0
var shouldHaveMorePollOptionsVotesNo = 0

for vote in shouldHaveMorePollOptionsVotes {
    if vote == true {
        shouldHaveMorePollOptionsVotesYes += 1
    } else {
        shouldHaveMorePollOptionsVotesNo += 1
    }
}

if shouldHaveMorePollOptionsVotesYes > shouldHaveMorePollOptionsVotesNo {
    print("Poll options must be more, because quantity of \"Yes-voters\" — \(shouldHaveMorePollOptionsVotesYes), more then \"No-voters\" — \(shouldHaveMorePollOptionsVotesNo)")
} else {
    print("Poll options can't be more, because quantity of \"Yes-voters\" — \(shouldHaveMorePollOptionsVotesYes), less or equal then \"No-voters\" — \(shouldHaveMorePollOptionsVotesNo)")
}
/*:
### Extension:
...
func printReults(forIssue: String , withVotes: [Bool]) -> String {
    var yesVotes = 0
    var noVotes = 0
...
printReults(forIssue: "Should we change the mascot?", withVotes: shouldMascotChangeVotes)
printReults(forIssue: "Should we install coffee vending machine?", withVotes: shouldInstallCoffeeVendingMachineVotes)
printReults(forIssue: "Should we have more poll options?", withVotes: shouldHaveMorePollOptionsVotes)
/*:
